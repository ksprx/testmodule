<?php
namespace App\Modules\Notification\Controllers\messengers\bale;
class api {
	...
	{
		$rawData =
			file_get_contents( "https://tapi.bale.ai/" . $this->token . '/setwebhook?url=' . $url );
			...
			if( array_key_exists( 'successful_payment' , $tmp_data[ 'message' ] ) ) {
				if( array_key_exists( 'order_info' , $tmp_data[ 'message' ][ 'successful_payment' ] ) ) {
					if( array_key_exists( 'user_id' , $tmp_data[ 'message' ][ 'successful_payment' ][ 'order_info' ] ) ) {
						$value =
							$tmp_data[ 'message' ][ 'successful_payment' ][ 'order_info' ][ 'user_id' ];
					}
				}
			}
	...
	{
		$tmp_data = $this->data;
		if( ! is_null( $tmp_data ) and ! is_null( $tmp_data )
		                               and array_key_exists( 'callback_query' , $tmp_data ) )
			$tmp_data = $tmp_data[ 'callback_query' ];
		if( ! is_null( $tmp_data ) and ! is_null( $tmp_data )
		                               and array_key_exists( 'message' , $tmp_data ) )
			if( array_key_exists( 'reply_to_message' , $tmp_data[ 'message' ] ) )
	...
	public function ForwardFromID() //if the message was a forward message, it will return ID of member who sent.
	{
		...
		if( ! is_null( $tmp_data ) and ! is_null( $tmp_data )
		                               and array_key_exists( 'message' , $tmp_data ) )
			if( array_key_exists( 'forward_from_chat' , $tmp_data[ 'message' ] )
			    or array_key_exists( 'forward_from' , $tmp_data[ 'message' ] ) )
	...
	public function Text() //return text or caption of the message
	{
		...
		if( ! is_null( $tmp_data ) and ! is_null( $tmp_data )
			...
			elseif( ! is_null( $tmp_data )
			        and array_key_exists( 'caption' , $tmp_data[ 'message' ] ) )
				return $tmp_data[ 'message' ][ 'caption' ];
			elseif( ! is_null( $tmp_data ) and ! is_null( $tmp_data )
			                                   and array_key_exists( 'edited_message' , $tmp_data ) )
				if( array_key_exists( 'text' , $tmp_data[ 'edited_message' ] ) )
					return $tmp_data[ 'edited_message' ][ 'text' ];
				elseif( ! is_null( $tmp_data )
				        and array_key_exists( 'caption' , $tmp_data[ 'edited_message' ] ) )
	...
	public function sendLocation( $arr ) {
		if( is_array( $arr ) ) {
			if( isset( $arr[ "chat_id" ] ) and isset( $arr[ "latitude" ] )
			                                   and isset( $arr[ "longitude" ] ) ) {
	...
	public function sendContact( $arr ) {
		if( is_array( $arr ) ) {
			if( isset( $arr[ "chat_id" ] ) and isset( $arr[ "phone_number" ] )
			                                   and isset( $arr[ "first_name" ] ) ) {
				$arr[ "command" ]      = 'sendLocation';
	...
	public function sendInvoice( $arr ) {
		if( is_array( $arr ) ) {
			if( isset( $arr[ "chat_id" ] ) and isset( $arr[ "title" ] )
			                                   and isset( $arr[ "description" ] )
			                                       and isset( $arr[ "provider_token" ] )
			                                           and isset( $arr[ "prices" ] )
			                                               and isset( $arr[ "payload" ] ) ) {
				...
				if( is_string( $arr[ 'prices' ] ) or is_int( $arr[ 'prices' ] )
				    or is_integer( $arr[ 'prices' ] ) or is_double( $arr[ 'prices' ] )
				    or is_float( $arr[ 'prices' ] ) )
					$arr[ 'prices' ] = [ [ "label" => "no lable" , "amount" => $arr[ 'prices' ] ] ];
				$return = "this" . $this->sendrequest( $arr );
			}
			else {
				$return =
					"undefined chatid or title or description or provider_token or prices or payload";
			}
		}
		else {
			$return = "it is not an array";
